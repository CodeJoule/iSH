...
By default the guest architecture is 32-bit x86 (i386). To build the AArch64 guest backend instead:

```
meson setup build-aarch64 -Dguest_arch=aarch64
ninja -C build-aarch64
```

Use Alpine aarch64 rootfs tarballs when running with `-Dguest_arch=aarch64`.
